...
await page.waitForTimeout(700);
await grab("/tmp/repairs.xlsx");

await page.goto(`${BASE}/quality`, { waitUntil: "networkidle" });
await page.waitForTimeout(700);
await grab("/tmp/quality-returns.xlsx");

await page.goto(`${BASE}/exports`, { waitUntil: "networkidle" });
await page.waitForTimeout(700);
const txt = await page.locator("main").innerText();
for (const name of ["أوامر الإصلاح", "المرتجعات"]) console.log(`مركز التصدير فيه «${name}»:`, txt.includes(name));
