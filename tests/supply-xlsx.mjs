...
for (const [tab, out] of [
  ["orders", "/tmp/supply-orders.xlsx"],
  ["batches", "/tmp/supply-batches.xlsx"],
  ["available", "/tmp/supply-available.xlsx"],
]) {
  await page.goto(`${BASE}/supply?tab=${tab}`, { waitUntil: "networkidle" });
  await page.waitForTimeout(700);
  await grab(out);
}
...
for (const name of ["أوامر التوريد", "دفعات الخامات", "المتاح فعلًا"]) {
