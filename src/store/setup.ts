...
export function setupGaps(db: Db, workspace: Workspace | null): SetupGap[] {
  ...
  if (noBom.length) {
    out.push({
      key: "bom",
      title: `${qty(noBom.length, 0)} موديل من غير قائمة خامات`,
  ...
  if (noRouting.length) {
    out.push({
      key: "routing",
      title: `${qty(noRouting.length, 0)} موديل من غير مسار تشغيل`,
  ...
  if (noPrice.length) {
    out.push({
      key: "price",
      title: `${qty(noPrice.length, 0)} خامة سعرها صفر`,
  ...
  if (pending.length) {
    out.push({
      key: "invites",
      title: `${qty(pending.length, 0)} دعوة لسه ماتقبلتش`,
  ...
  if (unlinked.length && db.orders.length > 0) {
    out.push({
      key: "delivery-order",
      title: `${qty(unlinked.length, 0)} توريد مش مربوط بأمر إنتاج`,
