...
export const DATASETS: DatasetDef[] = [
  ...
  {
    key: "machines",
    title: "الماكينات وجاهزيتها",
    about: "كل ماكينة: حالتها، وقّفت كام ساعة، جاهزيتها، أعطالها، تكلفة صيانتها، وميعاد صيانتها الجاي.",
    area: "production",
    module: "machines",
    screen: "/machines",
    groupBy: "line",
    cols: [
      code("code", "كود الماكينة"),
      text("name", "الماكينة", 22),
      text("kind", "النوع", 16),
      text("line", "الخط", 16),
      text("state", "الحالة", 14),
      count("plannedHours", "ساعات مخططة"),
      count("downHours", "ساعات توقف"),
      pct("availability", "الجاهزية"),
      pct("utilization", "نسبة التشغيل"),
      count("breakdowns", "أعطال"),
      count("mtbf", "بتعطل كل (ساعة)", "none"),
      count("mttr", "متوسط الإصلاح (ساعة)", "none"),
      cash("parts", "قطع غيار"),
      cash("labor", "أجر فني"),
      cash("outside", "ورشة خارجية"),
      cash("cost", "إجمالي الصيانة"),
      day("nextService", "الصيانة الجاية"),
    ],
    rows: (db) =>
      machineList(db).map((r) => ({
        id: r.machine.id,
        code: r.machine.code,
        name: r.machine.name,
        kind: MACHINE_KIND_LABEL[r.machine.kind],
        line: r.lineLabel,
        state: MACHINE_STATE_LABEL[r.machine.state],
        plannedHours: r.plannedMinutes / 60,
        downHours: r.downMinutes / 60,
        availability: r.plannedMinutes > 0 ? r.availabilityPct : null,
        ...
        parts: r.cost.parts,
        labor: r.cost.labor,
        outside: r.cost.outside,
        cost: r.cost.total,
        nextService: r.nextServiceOn ?? "",
      })),
    notes: [
      "الأرقام على آخر ٣٠ يوم، والزمن المخطط = دقايق الماكينة في اليوم × أيام العمل في تقويم الطاقة.",
      "«نسبة التشغيل» فاضية للماكينة اللي مافيش عمليات اتسجّلت عليها — الجاهزية بتتحسب من التوقف، ونسبة التشغيل بتتحسب من التسجيل.",
      "«بتعطل كل» و«متوسط الإصلاح» مش مجموعين: مجموع متوسطات على ماكينات مختلفة رقم بلا معنى.",
    ],
  },
  {
    key: "machine-tickets",
    title: "تذاكر الأعطال والصيانة",
    about: "كل تذكرة: الماكينة والسبب واللي اتعمل ووقت التوقف وتكلفتها بقطع الغيار.",
    area: "production",
    module: "machines",
    screen: "/machines/tickets",
    groupBy: "kind",
    cols: [
      code("code", "رقم التذكرة"),
      text("machine", "الماكينة", 22),
      text("line", "الخط", 16),
      text("kind", "النوع", 14),
      text("state", "الموقف", 18),
      day("reported", "تاريخ البلاغ"),
      text("cause", "السبب", 24),
      text("action", "اللي اتعمل", 24),
      text("technician", "الفني", 18),
      count("responseHours", "ساعات لحد البدء", "none"),
      count("downHours", "ساعات التوقف"),
      cash("parts", "قطع غيار"),
      cash("labor", "أجر فني"),
      cash("outside", "ورشة خارجية"),
      cash("cost", "إجمالي التذكرة"),
    ],
    rows: (db) =>
      ticketList(db).map((r) => ({
        id: r.ticket.id,
        ...
        line: r.line,
        kind: TICKET_KIND_LABEL[r.ticket.kind],
        state: TICKET_STATE_LABEL[r.ticket.state],
        reported: r.ticket.reportedOn,
        cause: r.ticket.cause,
        action: r.ticket.action,
        technician: r.technician,
        responseHours: r.responseHours,
        downHours: r.downMinutes / 60,
        parts: r.parts,
        labor: r.ticket.laborCost,
        outside: r.ticket.outsideCost,
        cost: r.cost,
      })),
    notes: [
      "التذكرة المفتوحة وقتها بيجري: بتتحسب من وقت بدء الإصلاح لحد دلوقتي، عشان الماكينة الواقفة تبان واقفة.",
      "قطع الغيار مأخوذة من حركات المخزن على التذكرة نفسها، مش رقم مكتوب.",
    ],
  },
  {
    key: "machine-downtime",
    title: "أسباب التوقف",
    about: "أكتر سبب وقّف المصنع وقت، بعدد مراته وساعاته وتكلفته — وعمود تراكمي.",
    area: "production",
    module: "machines",
    screen: "/machines?tab=downtime",
    cols: [
      text("cause", "السبب", 26),
      count("times", "مرات"),
      count("hours", "ساعات"),
      pct("share", "نسبة من التوقف"),
      pct("cumulative", "تراكمي"),
      cash("cost", "تكلفة"),
    ],
    rows: (db) =>
      downtimeCauses(db).map((c, i) => ({
        id: `cause-${i}`,
        cause: c.cause,
        times: c.count,
        hours: c.minutes / 60,
        share: c.sharePct,
        cumulative: c.cumulativePct,
        cost: c.cost,
      })),
    notes: ["الترتيب بالساعات مش بالعدد: أكتر سبب متكرّر مش شرط يكون أكتر سبب وقّف وقت."],
  },
  {
    key: "batches",
    title: "دفعات الخامات",
